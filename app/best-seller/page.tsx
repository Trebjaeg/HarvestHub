...
const BestSellerPageContent = () => {
  ...
  const [currentSort, setCurrentSort] = useState<string>('best_seller_rank');
  ...
  React.useEffect(() => {
    if (sorting?.current && currentSort === 'best_seller_rank') {
      setCurrentSort(sorting.current);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sorting]); // Intentionally excluding currentSort to prevent infinite loop

  // Initialize price range from configuration when available
  React.useEffect(() => {
    if (filtersConfig?.priceRange && priceRange[0] === 10 && priceRange[1] === 1500) {
      setPriceRange([filtersConfig.priceRange.min, filtersConfig.priceRange.max]);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filtersConfig]); // Intentionally excluding priceRange to prevent infinite loop
  ...
  const handleSortChange = (newSort: string) => {
    const sortOption = sorting?.options.find(opt => opt.id === newSort);
    if (sortOption) {
  ...
  const clearFilters = () => {
    ...
    setPriceRange([filtersConfig?.priceRange.min || 10, filtersConfig?.priceRange.max || 1500]);
    setSelectedRating(0);
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="hidden md:flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Link href="/cart" className="relative hover:bg-white/10 px-3 py-2 rounded-lg transition-all duration-200 hover:scale-105 flex items-center space-x-2">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="md:hidden">
            <div className="flex items-center justify-center text-sm">
              <div className="flex items-center space-x-3 text-xs overflow-x-auto">
                ...
                <Link href="/best-seller" className="font-medium py-2 border-b-2 whitespace-nowrap" style={{ color: '#614124', borderColor: '#614124' }}>Best Seller</Link>
          ...
          <div className="hidden md:flex items-center justify-center text-sm">
            <div className="flex items-center space-x-16">
              ...
              <Link href="/best-seller" className="font-medium py-2 border-b-2" style={{ color: '#614124', borderColor: '#614124' }}>Best Seller</Link>
              ...
              <div className="mb-8">
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      type="range"
                      min={filtersConfig?.priceRange.min || 10}
                      max={filtersConfig?.priceRange.max || 1500}
                      step={filtersConfig?.priceRange.step || 10}
                      value={priceRange[1]}
                      onChange={(e) => handlePriceRangeChange([priceRange[0], parseInt(e.target.value)])}
                      className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"
                    />
                    <div className="flex justify-between text-xs text-gray-500 mt-1">
                      <span>₱{filtersConfig?.priceRange.min || 10}</span>
                      <span>₱{filtersConfig?.priceRange.max || 1500}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 bg-white rounded-lg p-4 shadow-sm border border-gray-200">
              <div className="mb-4 md:mb-0">
                ...
                <p className="text-sm md:text-base text-gray-600" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  {loading ? 'Loading...' : `${totalProducts} best sellers found`}
                </p>
              </div>
              ...
              {loading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
                  {[...Array(12)].map((_, index) => (
                    <div key={index} className="border-2 border-gray-200 rounded-2xl bg-white shadow-sm overflow-hidden animate-pulse w-full max-w-[280px] mx-auto">
