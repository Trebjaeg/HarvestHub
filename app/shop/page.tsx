...
const ShopPageContent = () => {
  ...
  const [priceSortBy, setPriceSortBy] = useState('price-low-high');
  const [generalSortBy, setGeneralSortBy] = useState('newest');
  const [currentPage, setCurrentPage] = useState(1);
  const [showFilters, setShowFilters] = useState(false);

  const [activeSortType, setActiveSortType] = useState('general'); // 'price' or 'general'
  const activeSortBy = activeSortType === 'price' ? priceSortBy : generalSortBy;
      ...
      <header className="bg-[#103C2E] text-white py-3 md:py-4">
        <div className="container mx-auto px-4">
          ...
          <div className="hidden md:flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <Link href="/profile" className="flex items-center space-x-1 hover:bg-white/10 px-3 py-2 rounded-lg transition-all duration-200 hover:scale-105">
