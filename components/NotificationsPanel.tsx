...
export default function NotificationsPanel({ isOpen, onClose, onUnreadCountChange, buttonRef }: NotificationsPanelProps) {
  ...
  // Position below the notification bell (closer gap on mobile)
  // Allow an extra mobile multiplier so UI can be nudged further down when needed
  const MOBILE_BASE_GAP = 4; // px
  const MOBILE_DROP_MULTIPLIER = 9.5; // user requested "baba 0.5" -> 9+0.5=9.5x drop on mobile
  const MOBILE_LEFT_OFFSET_MULTIPLIER = 13; // user requested "left 1 times" -> 12+1=13x left shift on mobile
  const topPosition = buttonRect.bottom + (isMobile ? MOBILE_BASE_GAP * MOBILE_DROP_MULTIPLIER : 8);
  
  // For mobile: align right edge of panel with right edge of screen (with padding)
  // For desktop: align right edge of panel with right edge of button
  let leftPosition;
  
  if (isMobile) {
    // On mobile, position from right edge of screen
    // Apply left offset multiplier to shift panel further left
    const basePadding = 8;
    leftPosition = screenWidth - panelWidth - (basePadding * MOBILE_LEFT_OFFSET_MULTIPLIER);
  } else {
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        top: wouldOverflow && isMobile ? `${buttonRect.bottom + MOBILE_BASE_GAP * MOBILE_DROP_MULTIPLIER}px` : `${topPosition}px`,
